...
shinyServer(function(input, output, session) {
  ...
  ## All the data 
  craig_all <- get_via_csv(craig_ss)  %>% 
    select(-URL) %>% 
    mutate(DATE = as.Date(DATE, format = "%m/%d/%Y"), DESCRIPTION = stringr::str_to_lower(DESCRIPTION))
  
  ## Count number of Lost/found/stolen in each day
  craig_daily_types <- craig_all %>% 
    dplyr::mutate(Type = 
                    ifelse(stringr::str_detect(DESCRIPTION, "lost|missing"), "Lost",
                           ifelse(stringr::str_detect(DESCRIPTION, "found"), "Found", 
                                  ifelse(stringr::str_detect(DESCRIPTION, "stolen"), "Stolen", "Unknown")))) %>%
    dplyr::mutate(Type = Type %>% factor(c("Lost", "Found", "Stolen", "Unknown"))) %>%
    group_by(DATE) %>% count(DATE, Type) 
  ...
  dataInput1 <- reactive({
    ...
    craig_data <- craig_all %>% dplyr::filter(DATE == input$selectedDate)
    craig_data
  })
  ...
  dataInput2 <- reactive({
    ...
    craig_subset <- 
      craig_all[dplyr::between(craig_all$DATE, input$dateRange[1], input$dateRange[2]),] %>% 
      dplyr::mutate(COUNT = 1) %>% 
      dplyr::group_by(DATE) %>% 
      summarise(count = sum(COUNT))

    craig_daily <- 
      craig_daily_types[dplyr::between(craig_daily_types$DATE, 
                                       input$dateRange[1], 
                                       input$dateRange[2]),]
