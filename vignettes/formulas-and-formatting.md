...
*This is me taking notes to myself as I remember where I was on this when I dropped it in November. It will eventually become a proper vignette.*
...
When working with Google Sheets via [the cell feed](https://developers.google.com/google-apps/spreadsheets/data#work_with_cell-based_feeds), there are three ways to define cell contents:
  ...
  * **Literal value.** This is what hits your eyeballs when you view a Sheet in the browser. It's what `googlesheets` returns now.
...
The Sheet reading done above -- via `gs_read_csv()`, `gs_read_list()`, and `gs_download(..., to = "foo.csv") + csv import -- all assume that the data occupies a neat rectangle, that you want all of it, and that you want the literal values.
...
Let play with a modified version of `gs_read_cellfeed()`. As before, we return a data frame with one row per cell. But now, instead of `cell_text`, we return all 3 notions of cell contents:

  * `literal_value` (was `cell_text` in previous versions of `googlesheets`: what you see in the browser and what is returned by all other methods of reading
  * `input_value`: what you would have typed into the cell
  * `numeric_value`: the underlying number
...
Consider a formatted numeric column. All the existing read methods will be returning character. What is the right way to offer user the option of populating that column with `numeric_value` and getting a proper numeric variable?

Possible logic for redefining cell contents, prior to reshaping/simplification and type conversion:
...
Here's column 2, `Number_wFormat`, which holds formatted numbers. Compare `literal_value` (current default) and `numeric_value` (what people want).
...
#> 
#>    literal_value numeric_value
#>            (chr)         (chr)
#> 1 Number_wFormat            NA
#> 2        654,321      654321.0
#> 3         12.34%        0.1234
#> 4       1.23E+09  1.23456789E9
#> 5          3 1/7 3.14159265359
#> 6          $0.36          0.36
```

Here's column 4, `Formulas`, which has a bunch of formulas, which show up as such in `input_value`.
...
#>                                                     (chr)
#> 1                                                Formulas
#> 2           =HYPERLINK("http://www.google.com/","Google")
#> 3                              =sum(R[-1]C[-3]:R[3]C[-3])
...
#> 
#>     literal_value       numeric_value                   input_value
#>             (chr)               (chr)                         (chr)
#> 1 Formula_wFormat                  NA               Formula_wFormat
#> 2        3.18E+05           317897.75 =average(R[0]C[-4]:R[4]C[-4])
#> 3          52.63%  0.5263144432081239         =R[-1]C[-4]/R[1]C[-4]
#> 4            0.22 0.21739421366813996         =R[-2]C[-4]/R[2]C[-4]
#> 5      123,456.00            123456.0    =min(R[-3]C[-4]:R[1]C[-4])
#> 6         317,898           317897.75           =average(R2C1:R6C1)
```

Create an indicator if `numeric_value` does not exist. Create an indicator for an integer that is at risk of looking like a double if we take `numeric_value`. Create new putative cell content and replace `literal_value` with it. Drop helper variables.
...
Send this through existing `gs_reshape_cellfeed()`, which also does type conversion. Compare with existing read methods.
